...
from models.networks import Generator, LocalDis, GlobalDis
...
class Trainer(nn.Module):
    ...
    def forward(self, x, masks, ground_truth, compute_loss_g=False):
        ...
        # Keep the local branch on full-image tensors so free-form masks stay shape-stable.
        local_gt = ground_truth * masks
        local_x1_inpaint = x1_inpaint * masks
        local_x2_inpaint = x2_inpaint * masks

        # D part
        # wgan d loss
        local_patch_real_pred, local_patch_fake_pred = self.dis_forward(
            self.localD, local_gt, local_x2_inpaint.detach())
        ...
        if compute_loss_g:
            ...
            if sd_mask.shape[-2:] != local_gt.shape[-2:]:
                sd_mask = F.interpolate(sd_mask, size=local_gt.shape[-2:], mode='nearest')
            local_loss_weight = sd_mask * masks
            losses['l1'] = l1_loss(local_x1_inpaint * local_loss_weight, local_gt * local_loss_weight) * \
                self.config['coarse_l1_alpha'] + \
                l1_loss(local_x2_inpaint * local_loss_weight, local_gt * local_loss_weight)
            ...
            # wgan g loss
            local_patch_real_pred, local_patch_fake_pred = self.dis_forward(
                self.localD, local_gt, local_x2_inpaint)
            global_real_pred, global_fake_pred = self.dis_forward(
